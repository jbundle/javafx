 ...
 */
public class FBasePanel extends FScreenField
{
    /**
     ...
     */
    public FBasePanel(ScreenField model, boolean bEditableControl)
    {
        this();
    ...
    public void free()
    {
        ...
        if (m_ScreenLayout != null)
            m_ScreenLayout.free((FBasePanel)this);
        super.free();
        m_ScreenLayout = null;
    }
    /**
    ...
    {   // This code is the same as the code in BasePanel.Create (Eventually use the same code)
        ...
        {   // Alternate layout, calculate the size manually
            ...
            for (int i = 0; i < ((BasePanel)this.getScreenField()).getSFieldCount(); i++)
            {
                ...
                Rectangle rectField = ((FScreenField)sField.getScreenFieldView()).getControlExtent();
                if (rectField == null)
                    rectField = ((FScreenField)sField.getScreenFieldView()).calcBoxShape(ptLoc);
                else
                {
                    ptLoc.x = rectField.x;
                    ptLoc.y = rectField.y;
