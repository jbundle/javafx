    ...
    implements Runnable
{
    ...
    {
        return ((FBaseGridTableScreen)this.getScreenFieldView()).getModel();
    }
    /**
    ...
    public void handleUpdateMessage(BaseMessage message, int iChangeType)
    {
        ...
        { // This screen has changed, modify it in the table and on the screen
            ...
            ((FBaseGridTableScreen)this.getScreenFieldView()).fireTableRowsUpdated(iIndex, iIndex);
        }
    }
    /**
    ...
    public void handleSelectMessage(BaseMessage message)
    {
        ...
        int iSelection = ((FBaseGridTableScreen)this.getScreenFieldView()).getSelectedRow();   // Row to change
        int iHandleType = DBConstants.BOOKMARK_HANDLE;  // OBJECT_ID_HANDLE;
        ...
        if (iIndex != -1)
        {
            if (iSelection != -1)
            {
                ...
                if (boolUpdate.booleanValue())
                {
                    ...
                    ((FBaseGridTableScreen)this.getScreenFieldView()).tableChanged(iSelection);    // Refresh
