 ...
 */
public class FGridScreen extends FBaseGridTableScreen
    implements TableColumnModelListener
{
     ...
     */
    public FGridScreen(ScreenField model, boolean bEditableControl)
    {
        this();
    ...
    public void free()
    {
        ...
        if (table.isEditing())
            table.getCellEditor().stopCellEditing();	// Falidate any cell being edited.
    ...
    {   // Create the table
        ...
        for (int iIndex = 0; iIndex < gridScreen.getSFieldCount(); iIndex++)
        {
            ...
            Rectangle rect = ((FScreenField)sField.getScreenFieldView()).getControlExtent();
            if (rect == null)
                rect = ((FScreenField)sField.getScreenFieldView()).calcBoxShape(new Point(0, 0));
            iMaxHeight = Math.max(iMaxHeight, rect.height);
            TableColumn newColumn = new TableColumn(iIndex, rect.width, (FScreenField)sField.getScreenFieldView(), (FScreenField)sField.getScreenFieldView());
     ...
     * NOTE: The only component that would be hooked to a FGridScreen is a sort column value.
